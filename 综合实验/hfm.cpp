...
void quick_sort(head* header, int left, int right) {
    if (left >= right) return;
    int i = left, j = right;
    head pivot = header[left];
    while (i < j) {
    ...
    header[i] = pivot;
    quick_sort(header, left, i - 1);
    quick_sort(header, i + 1, right);
}
void quick_sort2(head *header, int left, int right) {
    if (left < right) {
        int i = left, j = right;
        head pivot = header[(left + right) / 2];
        while (i <= j) {
            while (strlen(header[i].bits) < strlen(pivot.bits)) i++;
            while (strlen(header[j].bits) > strlen(pivot.bits)) j--;
            if (i <= j) {
                head tmp = header[i];
                header[i] = header[j];
                header[j] = tmp;
        ...
        quick_sort(header, left, j);
        quick_sort(header, i, right);
    }
}
void compress() {
